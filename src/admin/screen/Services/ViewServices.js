...
import {
  ...
  Container,
  Searchbar,
} from "./ViewServices.styled";
import { SearchIcon, EditIcon, TrashIcons } from "../../assets/icons";
import AddComponents from "./AddServices";
import EditComponents from "./EditServices";
...
function ViewServices() {
  ...
  function getComponents() {
    const data1 = [];
    fetch(`${server_api}/api/getAvailableServices`)
  ...
  useEffect(() => {
    getComponents();
    getProductCategory().then((data) => setProductCategory(data));
  }, []);
  const sendRequest = async () => {
    const res = await axios
      .get(`${server_api}/api/deleteAvailableServices/${activeId}`)
      .catch((err) => console.log(err));
    const data = await res.data;
    return data;
  };
