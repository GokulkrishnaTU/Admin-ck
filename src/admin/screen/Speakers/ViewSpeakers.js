...
import {
  ...
  Container,
  Searchbar,
} from "./ViewSpeaker.styled";
import { SearchIcon, EditIcon, TrashIcons } from "../../assets/icons";
import AddComponents from "./AddSpeaker";
import EditComponents from "./EditSpeaker";
...
function ViewSpeakers() {
  ...
  function getComponents() {
    const data1 = [];
    fetch(`${server_api}/api/getSpeaker`)
      ...
      .then((response) => {
        response?.map((item, index) => {
          ...
          data1.push({
            id: item?.id,
            no: index + 1,
            speaker_type: item?.speaker_type_name,
  ...
  const sendRequest = async () => {
    const res = await axios
      .get(`${server_api}/api/deleteSpeaker/${activeId}`)
      ...
      <MenuBar>
        ...
        <MenuItems>
          <div
            ...
            <Button onClick={() => setOpen(true)}>Add Speaker Type</Button>
          </div>
        </MenuItems>
      </MenuBar>
      <TableContainer>
