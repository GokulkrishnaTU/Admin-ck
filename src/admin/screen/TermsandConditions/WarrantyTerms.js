...
const TermsandConditions = () => {
  ...
  const addDescription = () => {
    axios.post(`${server_api}/api/insertwarranty_terms`, {
      description: html
    }).then((res => {
  ...
  const updateDescription = () => {
    axios.post(`${server_api}/api/update_warrantyterms`, {
      id: description?.id,
  ...
  useEffect(() => {
    axios.get(`${server_api}/api/selectwarranty_terms`).then((res) => {
      setLoader(true)
  ...
  return (
    <Container>
...

      {
        ...
        </TermsDiv> : description ? <p>{parse(description?.description)}</p> : <TermsDiv>
          <EditorDiv>
