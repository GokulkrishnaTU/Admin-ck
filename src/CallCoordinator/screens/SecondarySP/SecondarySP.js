...
import {
  ...
  Container,
  Searchbar,
} from "./SecondarySP.styled";
import { SearchIcon, EditIcon, TrashIcons } from "../../assets/icons";
import AddVendor from "./AddSecondarySP";
import UpdateVendor from "./UpdateSecondarySP";
...
function SecondarySP() {
  ...
  function getServicePart() {
    ...
    fetch(`${server_api}/api/getVendor`)
      ...
      .then((response) => response.filter((prod) => prod.warranty == 1))
      .then((response) => {
        response?.map((item, index) => {
          ...
          data1.push({
            ...
            warranty_details: item?.warranty,
            action: (
              <ImageDiv>
                <img
                  src={EditIcon}
    ...
    <Container>
      <MenuBar>
        ...
        <MenuItems>
          ...
          >
            <Button onClick={() => setOpen(true)}>Add Secondary</Button>
          </div>
        </MenuItems>
      </MenuBar>
      <TableContainer>
